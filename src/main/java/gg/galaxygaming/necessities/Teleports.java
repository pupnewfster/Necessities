...
public class Teleports {
    ...
    public UUID lastRequest(UUID uuid) {
        return !teleportRequests.containsKey(uuid) ? null
              : UUID.fromString(teleportRequests.get(uuid).get(teleportRequests.get(uuid).size() - 1).split(" ")[0]);
    }
    ...
    public boolean hasRequestFrom(UUID uuid, UUID from) {
        return teleportRequests.containsKey(uuid) && (teleportRequests.get(uuid).contains(from.toString() + " toMe")
              || teleportRequests.get(uuid).contains(from.toString() + " toThem"));
    }
