...
public class Signs { //TODO Make sure this works, because some material names are long... so maybe use id if too long?
    ...
    public boolean checkFormat(Sign sign) {
        ...
        if (operation.equals("buy") || operation.equals("sell")) {
            ...
            return Utils.legalInt(amount)
                  && Necessities.getPrices().getPrice(operation, itemName, Integer.parseInt(amount)) != -1
    ...
    private String itemName(Sign sign) {
        String itemName = ChatColor.stripColor(sign.getLine(1).trim().replaceAll(" ", "")).replaceAll(":", " ")
              .split(" ")[0];
    ...
    public String itemLine(Sign sign) {
        ...
        return Material.fromString(itemName).getName() + (line.replaceAll(":", " ").split(" ").length > 1 ? ':' + line
              .replaceAll(":", " ").split(" ")[1] : "");
    }
    ...
    private void setPrice(Sign sign) {
        ...
        sign.setLine(3, Necessities.getVar().getMoney() + Economy.format(Necessities.getPrices()
              .getPrice(operation, Material.fromString(itemName).getName(), Integer.parseInt(sign.getLine(2).trim()))));
    }
