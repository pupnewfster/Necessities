        ...
        if (sender instanceof Player) {
            ...
            if (r instanceof ShapedRecipe) {
                ...
                for (Character c : s.getIngredientMap().keySet()) {
                    if (s.getIngredientMap().get(c) != null) {
                        items.put(s.getIngredientMap().get(c),
                              items.containsKey(s.getIngredientMap().get(c)) ? items.get(s.getIngredientMap().get(c))
                                    + 1 : 1);
                    }
                }
            ...
            for (Map.Entry<ItemStack, Integer> itemStackIntegerEntry : items.entrySet()) {
                Bukkit.broadcastMessage(
                      itemStackIntegerEntry.getValue() + " " + itemStackIntegerEntry.getKey().getType().toString());
            }
            Bukkit.broadcastMessage(
                  "results in " + r.getResult().getAmount() + ' ' + r.getResult().getType().toString());
        } else {
