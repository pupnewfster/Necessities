    ...
    public boolean commandUse(CommandSender sender, String[] args) {
        ...
        if (args.length < 3) {
            sender.sendMessage(var.getEr() + "Error: " + var.getErMsg()
                  + "Format requires you to enter a rank to create the rank that comes before and the rank that comes after.");
            return true;
        }
        ...
        if (!args[1].equalsIgnoreCase("null") && previous == null) {
            sender.sendMessage(
                  var.getEr() + "Error: " + var.getErMsg() + "The rank you inputted as previous does not exists.");
            return true;
        }
        if (!args[2].equalsIgnoreCase("null") && next == null) {
            sender.sendMessage(
                  var.getEr() + "Error: " + var.getErMsg() + "The rank you inputted as next does not exist.");
            return true;
        }
        if (args[1].equalsIgnoreCase("null") && args[2].equalsIgnoreCase("null") && !rm.getOrder().isEmpty()) {
            ...
            sender.sendMessage(var.getEr() + "Error: " + var.getErMsg()
                  + "You must put the new rank in between two consecutive ranks.");
            return true;
        }
