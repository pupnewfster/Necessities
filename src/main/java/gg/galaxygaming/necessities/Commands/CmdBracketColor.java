    ...
    public boolean commandUse(CommandSender sender, String[] args) {
        ...
        if (args.length == 0 || args.length > 1 && args[1].length() > 1 || args.length == 1 && args[0].length() > 1) {
            sender.sendMessage(var.getEr() + "Error: " + var.getErMsg()
                  + "You must enter a player and the color for their brackets.");
            sender.sendMessage(var.getMessages() + "Valid colors are: " + ChatColor
                  .translateAlternateColorCodes('&', "&00&11&22&33&44&55&66&77&88&99&aa&bb&cc&dd&ee&ff"));
            return true;
        }
        ...
        sender.sendMessage(
              var.getMessages() + "Bracket color changed to " + color + "this" + var.getMessages() + " for player "
                    + var.getObj() + target.getName());
        return true;
    }
