    ...
    public boolean commandUse(CommandSender sender, String[] args) {
        ...
        if (sender instanceof Player) {
            ...
            if (!p.hasPermission("Necessities.guilds.create")) {
                sender.sendMessage(
                      var.getEr() + "Error: " + var.getErMsg() + "You have not have permission to use /guild create.");
                return true;
            }
            if (args.length == 0) {
                sender.sendMessage(
                      var.getEr() + "Error: " + var.getErMsg() + "Must enter a name for the guild you wish to create.");
                return true;
            }
            ...
            if (config.contains("Necessities.Economy") && config.getBoolean("Necessities.Economy")) {
                ...
                if (eco.getBalance(u.getUUID()) < 800) {
                    sender.sendMessage(
                          var.getEr() + "Error: " + var.getErMsg() + "You do not have " + Economy.format(800)
                                + " to spend on creating a guild.");
                    return true;
                }
            }
            if (args[0].length() > 20) {
                sender.sendMessage(var.getEr() + "Error: " + var.getErMsg()
                      + "Your guild name is too long, the maximum length for a guild name is 20.");
                return true;
            }
            ...
            sender.sendMessage(
                  var.getMessages() + "Successfully created guild " + var.getObj() + args[0] + var.getMessages() + '.');
            if (eco != null) {
                eco.removeMoney(u.getUUID(), 800);
                sender.sendMessage(
                      var.getMoney() + Economy.format(800) + var.getMessages() + " was removed from your account.");
            }
        } else {
