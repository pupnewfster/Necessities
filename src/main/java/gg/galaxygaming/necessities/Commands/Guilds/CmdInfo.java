    ...
    public boolean commandUse(CommandSender sender, String[] args) {
        ...
        if (sender instanceof Player) {
            ...
            if (!p.hasPermission("Necessities.guilds.info")) {
                sender.sendMessage(
                      var.getEr() + "Error: " + var.getErMsg() + "You have not have permission to use /guild who.");
                return true;
            }
        ...
        if (other != null) {
            sender.sendMessage(
                  var.getMessages() + parant(name.length() / 2) + ".[ " + g.relation(other) + name + var.getMessages()
                        + " ]." + parant(name.length() / 2));
        } else {
            sender.sendMessage(
                  var.getMessages() + parant(name.length() / 2) + ".[ " + var.getNeutral() + name + var.getMessages()
                        + " ]." + parant(name.length() / 2));
        }
        ...
        sender.sendMessage(
              var.getMessages() + "Land / Power / Max power: " + var.getGuildMsgs() + g.getLand() + '/' + Utils
                    .roundTwoDecimals(g.getPower()) + '/' + g.getMaxPower() + ".00");
        if (!g.getAllies().isEmpty()) {
            StringBuilder alliesBuilder = new StringBuilder();
            for (String ally : g.getAllies()) {
                alliesBuilder.append(
                      other == null ? var.getNeutral() + ally + ", " : other.relation(gm.getGuild(ally)) + ally + ", ");
            }
        ...
        if (!g.getEnemies().isEmpty()) {
            ...
            for (String enemy : g.getEnemies()) {
                enemiesBuilder.append(other == null ? var.getNeutral() + enemy + ", "
                      : other.relation(gm.getGuild(enemy)) + enemy + ", ");
            }
        ...
        if (sender instanceof Player) {
            Player p = (Player) sender;
            p.sendMessage(var.getMessages() + "Members online: " + var.getNeutral() + g
                  .onlineMembers(p.hasPermission("Necessities.seehidden")));
            p.sendMessage(var.getMessages() + "Members offline: " + var.getNeutral() + g
                  .offlineMember(p.hasPermission("Necessities.seehidden")));
        } else {
