    ...
    public boolean commandUse(CommandSender sender, String[] args) {
        ...
        if (sender instanceof Player) {
            ...
            if (!p.hasPermission("Necessities.guilds.map")) {
                sender.sendMessage(
                      var.getEr() + "Error: " + var.getErMsg() + "You have not have permission to use /guild map.");
                return true;
            }
            ...
            if (owner != null) {
                name += owner.getName();
                sender.sendMessage(
                      var.getMessages() + parant(name.length() / 2) + ".[ " + owner.relation(u.getGuild()) + name + var
                            .getMessages() + " ]." + parant(name.length() / 2));
            } else {
                name += "Wilderness";
                sender.sendMessage(
                      var.getMessages() + parant(name.length() / 2) + ".[ " + var.getWild() + name + var.getMessages()
            ...
            for (int i = 0; i < chunks.size(); i++) {
                if (i % 39 == 0 && i != 0) {
                    if (row == 0) {
                        ...
                        sender.sendMessage(
                              line.toString() + var.getMessages() + ' ' + left(up) + var.getMessages() + "   " + right(
                                    up));
                    } else if (row == 2) {
            ...
            for (Map.Entry<Guild, String> guildEntry : symbols.entrySet()) {
                keyBuilder.append(
                      u.getGuild() == null ? var.getNeutral() + guildEntry.getValue() + ": " + guildEntry.getKey()
                            .getName() + ", "
                            : u.getGuild().relation(guildEntry.getKey()) + guildEntry.getValue() + ": " + guildEntry
                                  .getKey().getName() + ", ");
            }
            ...
            sender.sendMessage(var.getEr() + "Error: " + var.getErMsg()
                  + "You must be a player to view a map of claimed territories around you.");
        }
        return true;
    }
