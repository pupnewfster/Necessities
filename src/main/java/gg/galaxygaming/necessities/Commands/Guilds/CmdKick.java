    ...
    public boolean commandUse(CommandSender sender, String[] args) {
        ...
        if (sender instanceof Player) {
            ...
            if (!p.hasPermission("Necessities.guilds.kick")) {
                sender.sendMessage(
                      var.getEr() + "Error: " + var.getErMsg() + "You have not have permission to use /guild kick.");
                return true;
            }
            ...
            if (u.getGuild() == null || u.getGuild().getRank(p.getUniqueId()) == null || u.getGuild()
                ...
                sender.sendMessage(var.getEr() + "Error: " + var.getErMsg()
                      + "You must be a mod or higher in your guild to kick members.");
                return true;
            }
            if (args.length == 0) {
                sender.sendMessage(
                      var.getEr() + "Error: " + var.getErMsg() + "You must enter a player to kick out of your guild.");
                return true;
            }
            ...
            if (them.getGuild().getRank(uuid) == null || them.getGuild().getRank(uuid).equalsIgnoreCase("leader")) {
                sender.sendMessage(
                      var.getEr() + "Error: " + var.getErMsg() + "You may not kick the leader of your guild.");
                return true;
            }
            if (them.getGuild().getRank(uuid).equalsIgnoreCase("mod") && u.getGuild().getRank(p.getUniqueId())
                  .equalsIgnoreCase("mod")) {
                sender.sendMessage(
                      var.getEr() + "Error: " + var.getErMsg() + "You must be the leader of the guild to kick mods.");
                return true;
            }
            u.getGuild().kick(uuid);
            sender.sendMessage(
                  var.getMessages() + "Successfully kicked " + var.getObj() + them.getName() + var.getMessages()
                        + " from your guild.");
        } else {
            sender.sendMessage(
