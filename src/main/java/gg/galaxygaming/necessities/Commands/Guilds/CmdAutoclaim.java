        ...
        if (sender instanceof Player) {
            ...
            if (!p.hasPermission("Necessities.guilds.autoclaim")) {
                sender.sendMessage(var.getEr() + "Error: " + var.getErMsg()
                      + "You have not have permission to use /guild autoclaim.");
                return true;
            }
            User u = Necessities.getUM().getUser(p.getUniqueId());
            sender.sendMessage(var.getMessages() + (u.isClaiming() ? "No longer automatically claiming land."
                  : "Now automatically claiming land."));
