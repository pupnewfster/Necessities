        ...
        if (sender instanceof Player) {
            ...
            if (args.length > 2 || args.length == 0 || !Utils.legalDouble(args[1])) {
                sender.sendMessage(var.getEr() + "Error: " + var.getErMsg()
                      + "Format requires you enter a player and the amount you want to pay them.");
                return true;
            }
            String targetsName = "";
            ...
            if (!eco.doesPlayerExist(uuid)) {
                player.sendMessage(
                      var.getEr() + "Error: " + var.getErMsg() + "Please enter a valid player to send money to.");
                return true;
            }
            ...
            if (intBal < payAmount) {
                player.sendMessage(
                      var.getEr() + "Error: " + var.getErMsg() + "You don't have: " + var.getMoney() + Economy
                            .format(payAmount));
                return true;
            }
            ...
            player.sendMessage(
                  var.getMessages() + "You paid " + var.getObj() + targetsName + var.getMoney() + ' ' + Economy
                        .format(payAmount));
            if (target != null) {
                target.sendMessage(
                      var.getMessages() + "You received " + var.getMoney() + Economy.format(payAmount) + var
                            .getMessages() + " from " + var.getObj() +
                            player.getName() + var.getMessages() + '.');
            }
        } else {
            sender.sendMessage(
                  var.getEr() + "Error: " + var.getErMsg() + "You must be logged in to use this command or use cce");
        }
        return true;
    }
