...
public class CmdCmdPrices implements EconomyCmd {
    ...
    public boolean commandUse(CommandSender sender, String[] args) {
        ...
        if (sender instanceof Player) {
            ...
            while (price != null) {
                ...
                price = formL(Utils.capFirst(price.split(" ")[0]), price.split(" ")[1], price.split(" ")[2],
                      Integer.toString(page * 10 + time + 1) + '.', rank, hasNode);
            ...
            while (price != null) {
                price = formL(Utils.capFirst(price.split(" ")[0]), price.split(" ")[1], price.split(" ")[2],
                      Integer.toString(page * 10 + time + 1) + '.', "CONSOLE", true);
    ...
    private String formL(String cmd, String rank, String cost, String numb, String curRank, boolean hasCmd) {
        ...
        if (curRank.equals("CONSOLE")) {
            ...
            price = ChatColor.GOLD + numb + var.getCatalog() + cmd + " can be bought for " + var.getMoney() + cost + var
                  .getMessages() + " by " + rank + 's';
        } else {
            cost = Economy.format(Double.parseDouble(cost));
