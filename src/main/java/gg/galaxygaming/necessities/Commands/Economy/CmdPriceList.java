...
public class CmdPriceList implements EconomyCmd {
    ...
    public boolean commandUse(CommandSender sender, String[] args) {
        ...
        sender.sendMessage(
              ChatColor.GOLD + "Price List Page [" + Integer.toString(page) + '/' + Integer.toString(totalPages) + ']');
        page = page - 1;
        price = pr.priceLists(page, time);
        while (price != null) {
            price = formL(Material.fromString(price.split(" ")[0]).getFriendlyName(2), price.split(" ")[1],
                  price.split(" ")[2], Integer.toString(page * 10 + time + 1) + '.');
            sender.sendMessage(price);
            time++;
            price = pr.priceLists(page,
                  time);//TODO if I get all the prices for that page at once... then I can get the longest named item and try to line it up better
        }
        return true;
    }
