    ...
    public boolean commandUse(CommandSender sender, String[] args) {
        ...
        if (args.length < 2) {
            sender.sendMessage(var.getEr() + "Error: " + var.getErMsg()
                  + "Format requires you enter a player and the type of edit you want to do.");
        ...
        if (!eco.doesPlayerExist(uuid)) {
            sender.sendMessage(
                  var.getEr() + "Error: " + var.getErMsg() + "Please enter a valid player to change the balance of.");
            return true;
        }
        if (args[0].equalsIgnoreCase("reset")) {
            eco.setBalance(uuid, 0);
            sender.sendMessage(
                  var.getMessages() + "Your successfully reset the balance of " + var.getObj() + targetsName + var
                        .getMessages() + '.');
            return true;
        }
        if (args.length > 2) {
            if (!Utils.legalDouble(args[2])) {
                sender.sendMessage(var.getEr() + "Error: " + var.getErMsg()
                      + "You must enter a valid amount of money to modify the player with.");
                return true;
            }
            ...
            if (args[0].equalsIgnoreCase("give") || args[0].equalsIgnoreCase("add")) {
                ...
                sender.sendMessage(
                      var.getMessages() + "Your successfully gave " + var.getMoney() + ' ' + Economy.format(amount)
                            + var.getMessages() + " to " + var.getObj() + targetsName +
                            var.getMessages() + '.');
            } else if ((args[0].equalsIgnoreCase("take") || args[0].equalsIgnoreCase("remove"))
                  && intBal - amount >= 0) {
                eco.removeMoney(uuid, amount);
                sender.sendMessage(
                      var.getMessages() + "Your successfully took " + var.getMoney() + ' ' + Economy.format(amount)
                            + var.getMessages() + " from " + var.getObj() + targetsName +
                            var.getMessages() + '.');
            } else if (args[0].equalsIgnoreCase("set")) {
                eco.setBalance(uuid, amount);
                sender.sendMessage(
                      var.getMessages() + "Your successfully set the balance of " + var.getObj() + targetsName + var
                            .getMessages() + " to " + var.getMoney() + Economy.format(amount));
            }
        }
        return true;
    }
