    ...
    public boolean commandUse(CommandSender sender, String[] args) {
        ...
        if (args.length < 7) {
            sender.sendMessage(var.getEr() + "Error: " + var.getErMsg()
                  + "Format requires you enter a player a dimension to send them to an x coordinate a y coordinate a z coordinate and a yaw and pitch.");
            return true;
        }
        ...
        target.teleport(Necessities.getSafeLocations().getSafe(
              new Location(dim, Integer.parseInt(args[2]), Integer.parseInt(args[3]), Integer.parseInt(args[4]),
