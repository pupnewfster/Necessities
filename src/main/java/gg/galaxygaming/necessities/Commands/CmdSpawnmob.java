        ...
        if (sender instanceof Player) {
            ...
            if (args.length < 2) {
                sender.sendMessage(var.getEr() + "Error: " + var.getErMsg()
                      + "Format requires you enter a mob species to spawn and an amount to spawn.");
                return true;
            }
            ...
            p.sendMessage(
                  var.getMessages() + "Spawned " + var.getObj() + amount + var.getMessages() + " of " + var.getObj()
                        + type.toString().replaceAll("_", " ").toLowerCase() + var.getMessages() + '.');
        } else {
            sender.sendMessage(
                  var.getEr() + "Error: " + var.getErMsg() + "You cannot spawn mobs because you are... nice?");
        }
        return true;
    }
