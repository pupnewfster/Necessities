            ...
            if (args.length == 0) {
                ...
                if (config.contains("Necessities.Economy") && config.getBoolean("Necessities.Economy") && !free && config.contains("Necessities.Creative") && !config.getBoolean("Necessities.Creative")) {
                    if (eco.getBalance(p.getUniqueId()) < 2000) {
                        p.sendMessage(var.getEr() + "Error: " + var.getErMsg() + "You must have " + Economy.format(2000) + " to change your nick.");
                        return true;
                    }
                    eco.removeMoney(p.getUniqueId(), 2000);
                    p.sendMessage(var.getMoney() + Economy.format(2000) + var.getMessages() + " was removed from your account.");
                }
                if (uuid == null) {
                    User u = um.getUser(p.getUniqueId());
                    String nick = args[0];
        ...
        if (args.length == 1) {
            ...
            if (config.contains("Necessities.Economy") && config.getBoolean("Necessities.Economy") && !free && config.contains("Necessities.Creative") && !config.getBoolean("Necessities.Creative")) {
                if (eco.getBalance(target.getUniqueId()) < 2000) {
                    sender.sendMessage(var.getEr() + "Error: " + var.getErMsg() + "You must have " + Economy.format(2000) + " to change your nick.");
                    return true;
                }
