...
public class CmdClaim extends GuildCmd {
	...
	public boolean commandUse(CommandSender sender, String[] args) {
		if(sender instanceof Player) {
			...
			if(u.getGuild().getPower() != -1 && u.getGuild().getLand() >= u.getGuild().getPower() && !(u.getGuild().getPower() < 1)) {
			...
			if(args.length == 0 || args[0].equals("0") || !form.isLegal(args[0])) {
				...
				if(owner != null) {
					if(owner.equals(u.getGuild()))
						...
						if(owner.getPower() == -1) {
							sender.sendMessage(var.getEr() + "Error: " + var.getErMsg() + "This chunk cannot be claimed.");
							return true;
						}
						if(owner.isAlly(u.getGuild()) || owner.isNeutral(u.getGuild())) {
							sender.sendMessage(var.getEr() + "Error: " + var.getErMsg() + "You can only claim land from enemies not allies or neutral.");
							return true;
						}
						if(owner.getLand() > owner.getPower() || p.hasPermission("Necessities.guilds.admin")) {
							...
							u.getGuild().sendMods(var.getMessages() + "Successfully claimed chunk " + var.getObj() + p.getLocation().getChunk().getX() + var.getMessages() +
									", " + var.getObj() + p.getLocation().getChunk().getZ() + var.getMessages() + " in world " + var.getObj() + p.getWorld().getName() +
									var.getMessages() + " from the guild " + var.getObj() + owner.getName());
						} else
							sender.sendMessage(var.getEr() + "Error: " + var.getErMsg() + "This chunk is already claimed by another guild.");
					}
					return true;
				}
				u.getGuild().claim(p.getLocation().getChunk());
			...
			for(int i = 1; i < r; i++)
				for(int x = -i; x <= i; x++)
					for(int z = -i; z <= i; z++) {
						Chunk c = p.getWorld().getChunkAt(p.getLocation().getChunk().getX() + x, p.getLocation().getChunk().getZ() + z);
						...
						if(o != null) {
							if(!o.equals(u.getGuild()) && o.getPower() != -1 && ((o.getLand() > o.getPower() && !o.isAlly(u.getGuild()) &&
								...
								u.getGuild().sendMods(var.getMessages() + "Successfully claimed chunk " + var.getObj() + c.getX() + var.getMessages() + ", " +
										var.getObj() + c.getZ() + var.getMessages() + " in world " + var.getObj() + c.getWorld().getName() + var.getMessages() +
										" from the guild " + var.getObj() + o.getName());
							}
						} else {
							u.getGuild().claim(c);
							u.getGuild().sendMods(var.getMessages() + "Successfully claimed chunk " + var.getObj() + c.getX() + var.getMessages() + ", " +
									var.getObj() + c.getZ() + var.getMessages() + " in world " + var.getObj() + c.getWorld().getName());
						}
					}
		} else
