		...
		if (sender instanceof Player) {
			if(args.length != 5) {
				sender.sendMessage(var.getEr() + "Error: " + var.getErMsg() + "Format requires you to enter the player you wish to trade with " +
						"the item you wish to trade an how much the price to be payed and who pays.");
				return true;
			}
			...
			if(args[4].equalsIgnoreCase("theypay")) {
				...
				target.sendMessage(var.getObj() + pname + var.getMessages() + " has offered to trade you " + var.getObj() + amount + var.getMessages() + " of " +
						var.getObj() + item + var.getMessages() + " for " + var.getMoney() + "$" + form.addCommas(price));
			...
			if(args[4].equalsIgnoreCase("ipay")) {
				...
				item = mat.pluralize(mat.getName(item), Integer.parseInt(amount));
				player.sendMessage(var.getMessages() + "You have offered a trade to " + var.getObj() + offertopname);
				target.sendMessage(var.getObj() + pname + " has offered to trade you " + var.getMoney() + "$" + form.addCommas(price) + var.getMessages() + " for " +
						var.getObj() + amount + var.getMessages() + " of " + var.getObj() + item);
